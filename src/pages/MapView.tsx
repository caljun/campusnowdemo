...
function FlyToTarget({
  ...
  useEffect(() => {
    if (!target) return;
    // CSS transition is 300ms — wait 400ms for map to be fully sized
    setTimeout(() => {
      map.invalidateSize();
      map.flyTo([target.lat, target.lng], 18, { duration: 0.8 });
      // open popup after fly animation completes (~1s total)
      if (target.postId) {
        setTimeout(() => {
          markerRefs.current[target.postId!]?.openPopup();
        }, 900);
      }
      onFliedRef.current();
    }, 400);
  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        <FlyToTarget target={flyTarget} onFlied={onFlied} markerRefs={markerRefs} />
